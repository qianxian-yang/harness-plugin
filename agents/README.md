...
|-------|-----------|---------|------------------|
| **Skill** | A workflow with steps and exit criteria | `td-code-review-and-quality` | The *how* — invoked from inside a persona or command |
| **Persona** | A role with a perspective and an output format | `code-reviewer` | The *who* — adopts a viewpoint, produces a report |
| **Command** | A user-facing entry point | `/td-review`, `/td-ship` | The *when* — composes personas and skills |
...
- `/td-review` → wraps `code-reviewer` with the project's review skill
- `/td-test` → wraps `test-engineer` with TDD skill
...
- `/td-ship` → fans out to `code-reviewer` + `security-auditor` + `test-engineer` in parallel, then synthesizes their reports into a go/no-go decision
...
└── No  → Are the sub-tasks independent (no shared mutable state, no ordering)?
         ├── Yes → Slash command with parallel fan-out (e.g. /td-ship)
         └── No  → Sequential slash commands run by the user (/td-spec → /td-plan → /td-build → /td-test → /td-review)
```

## Worked example: valid orchestration

`/td-ship` is the canonical fan-out orchestrator in this repo:
...
- The user already knows they want a review; let them call `/td-review` directly
...
- **As subagents:** auto-discovered when this plugin is enabled (no path config needed). Use the Agent tool with `subagent_type: code-reviewer` (or `security-auditor`, `test-engineer`). `/td-ship` is the canonical example.
