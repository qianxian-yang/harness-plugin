...
`environment.json` is the contract between harness-creator and downstream execution/verification workflows. It describes everything needed to know to start the application, set up dependencies, and run verification — but it does NOT define what to verify. Verification configuration (`verify.json`) is generated at task runtime by the downstream workflow.

> **Key principle**: harness-creator answers "what does this project need to run?" — downstream workflows answer "what should we check after making changes?"
...
```json
{
  ...
  "scripts": {
    "setup": "harness/server-scripts/setup-env.sh",
    "start": "harness/server-scripts/start-server.sh",
    "teardown": "harness/server-scripts/teardown-env.sh"
  },
  "_meta": {
...
| 8 | **AskUserQuestion** | Required if all auto-detection fails |
...
|-------------|-------------|-----------|--------|
| Startup command | Often yes | **Yes** | Auto-detect → if fail, **AskUserQuestion immediately** |
| Health endpoint | Sometimes | **Yes** | Auto-detect → if fail, **AskUserQuestion** |
| Port | Usually | No | Auto-detect → default 8080 |
| Database type | Often | **Yes** (if code uses DB) | Auto-detect → if fail, **AskUserQuestion** |
...
# Generated by harness-creator, consumed by downstream workflows
set -euo pipefail
...
## Autonomous Mode (AskUserQuestion Not Available)
