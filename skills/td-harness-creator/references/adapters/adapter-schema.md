...
adapter:
  ...
  linter:
    ...
    # How to run dependency direction linting (lint-deps)
    run_command: string          # "go run scripts/lint-deps.go", "npx ts-node scripts/lint-deps.ts"
    # Optional setup/install commands for static quality tooling (lint-quality)
    quality_setup_commands: [string] | null
    # Ordered static quality commands for lint-quality wrapper to execute
    quality_run_commands: [string] | null
