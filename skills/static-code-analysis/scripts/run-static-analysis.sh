...
HARNESS_DIR="$ROOT/harness/report/static-analysis"
...
prepare_python_dependencies() {
...

  {
    ...
    echo "# Installed into harness/report/static-analysis/.venv by run-static-analysis.sh."
  ...
  if ! command_exists python3; then
    add_dependency_note "python3 is not available, so harness/report/static-analysis/.venv could not be created."
    write_dependency_notes
    return 0
  fi
  ...
  if [ -z "$venv_python" ]; then
    ...
    if ! python3 -m venv "$HARNESS_DIR/.venv" > "$HARNESS_DIR/venv-create.log" 2>&1; then
      add_dependency_note "Creating harness/report/static-analysis/.venv failed. See $HARNESS_DIR/venv-create.log."
      write_dependency_notes
      return 0
    fi
    venv_python="$(venv_bin "$HARNESS_DIR/.venv" python || true)"
  ...
  if [ -z "$venv_python" ]; then
    add_dependency_note "Created harness/report/static-analysis/.venv, but its python executable was not found."
    write_dependency_notes
    return 0
  fi
...
write_dependency_notes() {
  ...
  fi
  {
    ...
    echo
    echo "The script installs Python quality tools into \`harness/report/static-analysis/.venv\` using that virtual environment's pip."
