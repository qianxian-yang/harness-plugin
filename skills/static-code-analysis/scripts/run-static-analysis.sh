...
if [ -n "$OUTPUT_PATH" ]; then
  ...
  REPORT_PATH="$HARNESS_DIR/static-analysis-report.html"
fi
...
has_python_files() {
  find "$ROOT" \
    \( -name .git -o -name node_modules -o -name .venv -o -name venv -o -name target -o -name build -o -name dist -o -name __pycache__ -o -path "$HARNESS_DIR" \) -prune \
...
prepare_python_dependencies() {
  ...
  if ! python_tool ruff >/dev/null && ! python_tool flake8 >/dev/null; then
    echo "ruff" >> "$requirements_tmp"
    add_dependency_note "\`ruff\`: Python files found, but neither Ruff nor Flake8 is available."
  fi
  ...
  if { has_file "mypy.ini" || has_config_text mypy pyproject.toml setup.cfg; } && ! python_tool mypy >/dev/null; then
    echo "mypy" >> "$requirements_tmp"
    add_dependency_note "\`mypy\`: mypy config found, but mypy is not available."
  fi
...
discover_tools() {
  ...
  if has_python_files; then
    ...
    elif flake8_path="$(python_tool flake8)"; then
      add_tool "Flake8" 0 "flake8 available" "$flake8_path" .
    else
      add_skipped "Python files found, but neither ruff nor flake8 is available"
    fi
    ...
    if { has_file "mypy.ini" || has_config_text mypy pyproject.toml setup.cfg; } && mypy_path="$(python_tool mypy)"; then
      add_tool "mypy" 0 "mypy config found" "$mypy_path" .
    fi
...
run_tools() {
  ...
  while IFS="$(printf '\t')" read -r name fail_on_output reason command_text || [ -n "$name" ]; do
    ...
    if [ "$exit_code" -eq 0 ]; then
      status="passed"
      status_html='<span class="badge pass">passed</span>'
    else
      status="failed"
      status_html='<span class="badge fail">failed</span>'
    ...
    escaped_reason="$(printf '%s' "$reason" | html_escape)"
    {
      echo '<tr>'
      echo "<td>$escaped_name</td><td><code>$escaped_command</code></td><td>$status_html</td><td>$exit_code</td><td>${duration}s</td><td>$escaped_reason</td>"
      echo '</tr>'
    } >> "$TOOL_ROWS_FILE"
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .meta, .notice, .panel, .finding, details { background: var(--panel); border: 1px solid var(--border); border-radius: 8px; padding: 16px; margin-top: 14px; }
    ...
    .finding { border-left: 4px solid var(--fail); }
    .finding-head { display: flex; justify-content: space-between; gap: 16px; flex-wrap: wrap; }
...
HTML
    ...
    echo '    <table><thead><tr><th>Tool</th><th>Command</th><th>Status</th><th>Exit</th><th>Duration</th><th>Why it ran</th></tr></thead><tbody>'
    if [ -s "$TOOL_ROWS_FILE" ]; then
      cat "$TOOL_ROWS_FILE"
    else
      echo '<tr><td colspan="6">No compatible static analysis tools were detected.</td></tr>'
    fi
    echo '    </tbody></table>'
