...
│  - Documentation Agent: AGENTS.md, harness/docs/*                          │
│  - Linter Agent: harness/lint-scripts/lint-*                                    │
│  - Config Agent: harness/*, harness/Makefile                               │
└─────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────┐
...
has_architecture=$(test -f harness/docs/ARCHITECTURE.md && echo "yes" || echo "no")
has_linters=$(ls harness/lint-scripts/lint-* 2>/dev/null | wc -l)
has_harness_dir=$(test -d harness && echo "yes" || echo "no")
has_makefile=$(test -f harness/Makefile && echo "yes" || echo "no")
  ...
  "options": [
    {
      ...
      "description": "Complete setup: AGENTS.md, docs, linters, eval framework, runtime scripts"
    },
    {
      "label": "Documentation only",
      "description": "Just AGENTS.md + harness/docs/ for now, add linters/evals later"
    },
    {
...
2. Check harness/docs/: ARCHITECTURE.md accuracy, design docs coverage
3. Check harness/lint-scripts/lint-*: layer map coverage, error message quality
...
- [ ] harness/lint-scripts/lint-deps.go
- [ ] harness/config/environments.json

### To Update (exists but has gaps)
- [ ] harness/docs/DEVELOPMENT.md — missing build commands
- [ ] harness/lint-scripts/lint-quality.py — missing 3 packages in layer map

### Already Good (no changes needed)
- [x] harness/Makefile — has all required targets
```
...
2. harness/docs/ARCHITECTURE.md — Mermaid diagrams from actual imports, layer table
3. harness/docs/DEVELOPMENT.md — Real build/test/lint commands
4. harness/docs/design-docs/*.md — For each key component
...
Files to create/update:
1. harness/lint-scripts/lint-deps.{ext} — Layer map with ALL packages from analysis
2. harness/lint-scripts/lint-quality.{ext} — Code quality rules
...
Files to create/update:
1. harness/config/environments.json — Runtime ecosystem contract (v2.0 schema)
2. harness/server-scripts/setup-env.sh — Start dependencies
3. harness/server-scripts/start-server.sh — Start app
4. harness/server-scripts/teardown-env.sh — Cleanup
5. harness/Makefile targets — lint-arch, build, test
...
environments.json + task context (which files changed, what the task was).
...
### 4.2 Wait for Creation Completion
...
test -f harness/docs/ARCHITECTURE.md && echo "✓ ARCHITECTURE.md"
test -f harness/lint-scripts/lint-deps* && echo "✓ lint-deps"
test -d harness/ && echo "✓ harness/"

# 5. Design docs exist (not just index)
find harness/docs/design-docs -name "*.md" ! -name "index.md" | wc -l
```
...
- harness/docs/design-docs/{component}.md
...
### Next Steps
{For empty projects: "Run harness-executor to implement business code from harness/docs/exec-plans/active/bootstrap-code.md"}
...
With context: "Implement the bootstrap exec-plan at harness/docs/exec-plans/active/bootstrap-code.md"
