...
#!/usr/bin/env python3
"""
Language-native static quality checks for Python.
...
FALLBACK_VENV = Path("/Users/yqx/ai/aitroll/venv/bin")


def _find_venv_bin() -> Path | None:
    candidates = [
        REPO_ROOT / "venv" / "bin",
        REPO_ROOT / ".venv" / "bin",
        FALLBACK_VENV,
    ...
    for candidate in candidates:
        if (candidate / "ruff").exists() and (candidate / "mypy").exists():
            return candidate
    return None


def _run(cmd: list[str], *, tool_name: str) -> int:
    print("+", " ".join(cmd))
    result = subprocess.run(cmd, cwd=REPO_ROOT, check=False)
    if result.returncode != 0:
        sys.stderr.write(
            f"\n{tool_name} failed (exit={result.returncode}).\n"
            f"Fix the diagnostics above and rerun scripts/lint_quality.py.\n"
        )
...
def main() -> None:
    ...
    if venv_bin is None:
        if shutil.which("ruff") and shutil.which("mypy"):
            ruff_cmd = ["ruff", "check", "."]
            mypy_cmd = ["mypy", "."]
        else:
            sys.stderr.write(
                "Missing required static tools: ruff + mypy.\n"
                "Install with one of:\n"
        ...
        ruff_cmd = [str(venv_bin / "ruff"), "check", "."]
        mypy_cmd = [str(venv_bin / "mypy"), "."]

    rc = _run(ruff_cmd, tool_name="ruff")
    if rc != 0:
        return rc

    rc = _run(mypy_cmd, tool_name="mypy")
    if rc != 0:
        return rc

    print("✓ Python static quality checks passed (ruff + mypy)")
    return 0
