---
name: "source-command-td-build"
description: "Implement the next task incrementally — build, test, verify, commit"
---

# source-command-td-build

Use this skill when the user asks to run the migrated source command `td-build`.

## Command Template

Invoke the harness-skills:td-incremental-implementation skill alongside harness-skills:td-test-driven-development.

Pick the next pending task (unchecked `- [ ]`) from the Tasks section in `docs/td-harness/YYYY-MM-DD-<feature-name>/plan.md` (reuse the directory created by `/td-spec` and `/td-plan`). For each task:
