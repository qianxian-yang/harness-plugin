...
if (hasJq) {
  ...
  if (!payload.message.includes('td-skill-navigation loaded.')) {
    throw new Error('message is missing startup preface');
  }

  if (!payload.message.includes('# TD Skill Navigation') && !payload.message.includes('td-skill-navigation')) {
    throw new Error('message is missing skill-navigation content');
  }
} else {
