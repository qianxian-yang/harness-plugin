# Using td-harness with Gemini CLI
...
gemini skills install /path/to/td-harness/skills/
```

**Install for a specific workspace only:**

```bash
gemini skills install /path/to/td-harness/skills/ --scope workspace
```
...
cat /path/to/td-harness/skills/test-driven-development/SKILL.md > GEMINI.md
echo -e "\n---\n" >> GEMINI.md
cat /path/to/td-harness/skills/code-review-and-quality/SKILL.md >> GEMINI.md
```
...
> **Skills vs GEMINI.md:** Skills are on-demand expertise that activate only when relevant, keeping your context window clean. GEMINI.md provides persistent context loaded for every prompt. Use skills for workflow-specific guidance and GEMINI.md for always-on project conventions.
...
- `test-driven-development` — Implement with failing tests first
...
- `api-and-interface-design` — Activates when designing API contracts or boundaries
...
Many skills in this pack leverage [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) tools to interact with the environment. For example, `performance-optimization` can benefit from performance-related MCP tools.
...
To replicate the `td-harness` experience from other tools, you can configure a `SessionStart` hook that reminds you of the available skills or loads a meta-skill.
